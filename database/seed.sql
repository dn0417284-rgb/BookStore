-- seed.sql (đã sửa lỗi)
-- ============================================================
-- Lưu ý: password dưới đây là bcrypt hash của chuỗi tương ứng
--   admin123  → hash bên dưới
--   customer1 → hash bên dưới
-- Bạn cũng có thể tự tạo hash bằng script hash-passwords.js
-- ============================================================

-- Xóa dữ liệu cũ (nếu cần reset)
-- TRUNCATE TABLE customers;

-- Thứ tự cột đúng: (full_name, email, password, phone, address, role, account_status, warning_count)

INSERT INTO customers (full_name, email, password, phone, address, role, account_status, warning_count)
VALUES
  -- Tài khoản ADMIN để test — password: admin123
  (
    'Quản Trị Viên',
    'admin@bookstore.com',
    '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', -- bcrypt(admin123)
    '0900000000',
    'Hà Nội',
    'admin',
    'active',
  ...
  -- Khách hàng mẫu 1 — password: customer1
  (
    'Nguyễn Văn An',
    'an@example.com',
    '$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lh3y', -- bcrypt(customer1)
    '0901234567',
    'TP. Hồ Chí Minh',
    'customer',
    'active',
  ...
  -- Khách hàng mẫu 2 — password: customer1
  (
    'Trần Thị Bình',
    'binh@example.com',
    '$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lh3y', -- bcrypt(customer1)
  ...
  -- Khách hàng mẫu 3 — đã bị cảnh báo 1 lần
  (
    'Lê Minh Cường',
    'cuong@example.com',
...
-- ============================================================
-- Chạy file này:  mysql -u root -p bookstore_db < seed.sql
-- ============================================================
-- -- customers
